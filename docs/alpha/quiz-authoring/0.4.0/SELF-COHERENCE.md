...
| AC3 | `PATCH /quizzes/:id` updates title/description/status, valid transition draft→published, 400 for invalid status, 403 if not author | `quiz.service.ts:updateQuiz` — validates against `VALID_STATUSES` set, checks `PUBLISHED→non-PUBLISHED` guard. Tests: "allows draft→published transition", "throws 400 for invalid status value", "throws 400 for published→draft transition", "throws 403 when requester is not the author" |
