...
Before sending: verify PR number (`gh pr list`) and issue number (`gh issue list`) are correct. Transposed numbers silently misdirect β.
...
**After β approves:** immediately check whether `.cdd/releases/{version}/alpha/CLOSE-OUT.md` exists on main. If not, dispatch α for close-out before beginning triage — do not wait to discover it missing during the closure gate check.
