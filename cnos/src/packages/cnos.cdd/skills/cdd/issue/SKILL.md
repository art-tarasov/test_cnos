...
2.2.5. **State machine exhaustiveness: name terminal states in invariants, not only valid transitions in ACs**
  - Listing "valid transitions: A → B" implies A is terminal but does not state it. An implementer who reads only the validation sentence ("Returns 400 for invalid value") may guard against the specific bad case rather than enforcing the only-good path.
  - Move terminal-state facts to the active invariants section, stated as rules: "X is a terminal state — no further transitions are valid."
  - ❌ AC says "Valid status transitions: draft → published" and separately "Returns 400 for invalid status value" — implementer may block downgrade but allow same-state no-op
  - ✅ AC says "Valid status transitions: draft → published"; invariant says "published is terminal — any status update when published returns 400"
...
2.4.1. **Name the Tier 3 (issue-specific) skills the implementer must load**
  ...
  - Name only the issue-specific skills: language (`eng/<language>`), domain (`eng/ux-cli`, `eng/performance-reliability`), architecture (`eng/architecture-evolution`), etc.
