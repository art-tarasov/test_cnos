...
2. PR body carries CDD Trace through step 7; if the trace claims bootstrap was "first diff on branch," verify at least one commit exists containing only the version directory before implementation commits — if bootstrap and implementation landed in one commit, the trace must state that honestly (not claim ordered bootstrap)
